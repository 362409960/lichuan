...
public class QRCode {
	...
	public static void main(String[] args) throws WriterException {
		try {
			getLogoQRCode("http://www.qq.com/", null, null);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public static String getLogoQRCode(String qrUrl, HttpServletRequest request, String productName) {
		...
		String filePath = "d:/lichuan/2.png"; // TODO
		...
		try {
			...
			return zp.addLogo_QRCode(bim, new File(filePath), new QRUtils(), productName);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	public String addLogo_QRCode(BufferedImage bim, File logoPic, QRUtils qRUtils, String productName) {
		try {
			/** * 读取二维码图片，并构建绘图对象 */
			...
			ImageIO.write(image, "png",
					new File("D:/lichuan/TDC-" + new Date().getTime() + "test.png")); // TODO

			//String imageBase64QRCode =  Base64.byteArrayToBase64(baos.toByteArray());
			
			String imageBase64QRCode = baos.toByteArray().toString();
			baos.close();
