...
/**
 * Simple HTTP server. It can process certain request according to a given
 * service. This servers acts even as a torrent file depository or a tracker for
 * bittorrent client
 *
 * <p>
 * Titre : TrackerBT
...
public class Tracker {
	...
	public static void main(String[] args1) {
		String[] args = {"E://Workspaces//bt//src//main//java//tracker_config.xml"};
		String rootPath= Tracker.class.getResource("").getFile().toString(); 
		...
		if (args.length > 0) {
			...
			System.err.println("Please specify the location of the configuration file");
			System.exit(0);
		}
		new File((String) Constants.get("context")).mkdirs();
		try {
			FileWriter fw = new FileWriter((String) Constants.get("context") + "Mapper.xml");
		...
		try {
			...
			System.out.println("Tracker started! Listening on port " + Constants.get("listeningPort")
					+ "\r\n\r\n\t********************************************\r\n"
					+ "\t*   Press enter to shut down the server    *\r\n"
					+ "\t********************************************\r\n");
			// Setup the I/O buffered stream, to read user input from the
			// command line
