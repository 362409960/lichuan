...
public class TrackerService extends Service {
        ...
        if(temp.length >= 2){
            ...
            for(int i = 0; i < param.length; i++){
                ...
                if(splitParam.length == 1)
                    ...
                    params.put(splitParam[0], URLDecoder.decode(splitParam[1], Constants.BYTE_ENCODING));
            }
            return params;
        }
        return null;
        ...
        try {
            ...
            case 0:
                ...
                for (Iterator it = l.iterator(); it.hasNext(); ) {
                    ...
                    String s = URLDecoder.decode(Utils.byteStringToByteArray(e.getChild("id").getText()), Constants.BYTE_ENCODING);
                    peer.put("peer_id", s);
                    
                    String port = e.getChild("port").getText();
                    if(port == null || "".equals(port.trim()) ||"-1".equals(port)){
                    	return null;
                    }
                    
                    peer.put("port", new Integer(e.getChild("port").getText()));
                    if(logger.isDebugEnabled()){
                    	logger.debug("peer port: "+new Integer(e.getChild("port").getText()));
                    }
	...
	private boolean isNewTorrent(Element root, String hash) {
        for (Iterator<?> hashIt = root.getDescendants(new ElementFilter("hash")); hashIt.hasNext(); ) {
    ...
    private void cleanPeerList(Element root) {
        for (Iterator it = root.getDescendants(new ElementFilter("peer")); it.hasNext(); ) {
            Element e = (Element) it.next();
     ...
     */
    private void registerPeer(Element root, String id, String ip, String port, String hash) {
        try {
            if (id == null || ip == null || port == null ||  "-1".equals(port) || hash == null) {   //"-1".equals(port)  update by huang_xc@sohu.com 20160511
                return;
            }
            Element peer = null;
            for (Iterator peerIt = root.getDescendants(new ElementFilter("peer")); peerIt.hasNext(); ) {
                peer = (Element) peerIt.next();
                if (peer.getChild("id").getText().matches(id) || (peer.getChild("ip").getText() + peer.getChild("port").getText()).matches(ip+port)) {
                    break;
                }
                peer = null;
            }
            if (peer != null) {
                ...
                for (Iterator it = peer.getDescendants(new ElementFilter( "torrentid")); it.hasNext(); ) {
                ...
                peer.setAttribute("updated", new Long(System.currentTimeMillis()).toString());
                peer.getChild("updated").setText(new Long(System.currentTimeMillis()).toString());
            } else {
    ...
    private List peerSharing(Element root, String hash, String peerid) {
        List peers = new ArrayList();
        for (Iterator hashIt = root.getDescendants(new ElementFilter("torrentid")); hashIt.hasNext(); ) {
    ...
    private boolean torrentExists(String id) {
        ...
        System.out.println("--------------------torrentXML-----------------------> "+torrentXML);
     ...
     */
    public synchronized int processPeerList(Map param, List peers) {
        SAXBuilder sb = new SAXBuilder();
        try {
            ...
            String hash = Utils.byteArrayToByteString(hashRaw.getBytes(Constants.BYTE_ENCODING));
            String peerid = Utils.byteArrayToByteString(peeridRaw.getBytes(Constants.BYTE_ENCODING));
            ...
            if (root != null) {
                ...
                this.registerPeer(root, peerid, (String) param.get("ip"), port, hash);
