...
public class BEncoder {
	...
	private void encode(ByteArrayOutputStream baos, Object object) throws IOException {
		...
		if (object instanceof String || object instanceof Float) {
			...
			// unfortunately there are some occasions where we want to ensure
			// that
			// the 'key' of the map is not mangled by assuming its UTF-8
			// encodable.
			...
			// to allow the constructor of the Map to indicate that the keys
			// should
			// be extracted using a BYTE_ENCODING

			boolean byte_keys = false; // object instanceof
			...
			baos.write('i');
			write(baos,
					Constants.DEFAULT_CHARSET.encode(tempInteger.toString()));
			baos.write('e');

		} else if (object instanceof byte[]) {

			byte[] tempByteArray = (byte[]) object;
			write(baos, Constants.DEFAULT_CHARSET.encode(String
					.valueOf(tempByteArray.length)));
			baos.write(':');
