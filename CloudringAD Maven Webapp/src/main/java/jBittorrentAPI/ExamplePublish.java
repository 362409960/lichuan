...
class ExamplePublish{
    public static void main(String[] args){
        ...
        for(int i = 4; i < args.length; i++)
            comment += args[i];
        try{
        	//                        example/client1/funvideo.torrent    http://localhost:8081/upload none      none                     "this is a fun video"
            ConnectionManager.publish(args[0],                            args[1],                     args[2],  args[3], f.getName(), "", comment,               "7");
        }catch(Exception e){
            System.out.println(showHelp());
            System.exit(0);
        }
