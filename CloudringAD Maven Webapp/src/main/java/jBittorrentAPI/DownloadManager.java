...
public class DownloadManager implements DTListener, PeerUpdateListener,
    ...
    public void blockUntilCompletion() {
        ...
        while (true) {
            ...
            //中断当前线程  update by huangxc
            long end = System.currentTimeMillis();
            System.out.println("blockUntilCompletion ---------------------> end - start  "+(end - start));
            
			 //测试10分钟结束该线程s钟
			 //if(((end - time)/1000)>60*60*24*10){
            ...
            //10天后，结束该线程
			 if(((end - start)/1000)>60*60*24*10){
				 ...
				 thread.suspend();
				 thread.stop();
				 System.out.println("**********************************终止 "+Thread.currentThread()+" 线程**************************");
			 }
        }
        /*
    ...
    public void startTrackerUpdate() {
        ...
        int tempPort = this.cl.getConnectedPort();
        if(tempPort == -1){
        	boolean f = this.cl.connect(cl.getMinPort(), cl.getMaxPort());
        	int count = 0;
        	while(!f && count < 100 ){
        		f = this.cl.connect(cl.getMinPort(), cl.getMaxPort());
        		count ++;
        ...
        System.out.println("this.cl.getConnectedPort()--------------------------->  "+this.cl.getConnectedPort());
