...
public class PeerUpdater extends Thread {
	...
	public void run() {
		...
		while (!this.end) {
			...
			this.peerList = this.processResponse(this.contactTracker(id, torrent, this.downloaded, this.uploaded, this.left, this.event));
			if (peerList != null) {
				if (first) { 
					this.event = "";
					first = false;
				...
				if (peers instanceof List) {
					...
					if (peerList != null && peerList.size() > 0) {
						for (int i = 0; i < peerList.size(); i++) {
							...
							System.out.println("------------------------> peerID is "+peerID+" ipAddress is "+ipAddress+"  port is "+port);
	 ...
	 */
	public synchronized Map contactTracker(byte[] id, TorrentFile t, long dl,
			long ul, long left, String event) {
		try {
			System.out.println("--------------------------- perrUpdate listeningPort is--------------------------- "+listeningPort);
	...
	/**
	 * Stops the update process. This methods sends one last message to the
	 * tracker saying this client stops sharing the file and it also exits the
	 * run method
	 */
	public void end() {
	 ...
	 * communication with this object
	 * 
	 * @param listener
	 *            PeerUpdateListener
	 */
	 ...
	 * representing the reason why the last try to contact tracker failed
	 * 
	 * @param error
	 *            int
	 * @param message
