...
public class PublishServiceImpl implements PublishService {
	 ...
	 */
	@Override
	public String savePublish(HttpServletRequest request, Publish publish)
			throws Exception {
		...
		for(String prId:programIds){		
			...
			// 
			//生成bt文件
			//String bitFile=btService.btHandle(user.getId(), zipFileName, zipFileName, user.getUserCode());
			//换成http开头的。
			//String bif=trackerBT.Constants.configParam.get("url")+File.separator +bitFile.split("webapps")[1];
			String bif=trackerBT.Constants.configParam.get("url")+File.separator+"example"+File.separator+user.getId()+File.separator+zipFileName; 
			...
			if(iscount>0){				
				...
				//bifUpdateFile=btService.btHandle(user.getId(), zipUpdateFileName, zipUpdateFileName, user.getUserCode());
				//bifUpdate=trackerBT.Constants.configParam.get("url")+File.separator +bifUpdateFile.split("webapps")[1];
				
				bifUpdate=trackerBT.Constants.configParam.get("url")+File.separator +"example"+File.separator+user.getId()+File.separator+zipUpdateFileName;
			...
			for(String id:terminal_ids){
				...
				logger.info("-------------------------> "+topic);
				String message = "{\"action\":\"publish\",\"url\":\""+bif+"\",\"id\":\""+programId+"\",\"addUrl\":\""+bifUpdate+"\",\"version\":"+version+"}";
	 ...
	 */
	private  static void BuildXMLDoc(String file,Program program,List<ProgramDetails> pdList,Publish publish) throws IOException, JDOMException {  
    	   ...
    	   for(ProgramDetails pd:pdList){
    		   if(!"".equals(pd.getVideoPlace_url())&&pd.getVideoPlace_url()!=null){    			  
    			   ...
    			   for(int i=0;i<top.length;i++){
    				   ...
    				   place.setAttribute("url", pd.getVideoPlace_url().replaceAll("/CloudringAD/images/sys", "SceneImages"));
	...
	private static void createHtml(String fileName,Program program,ProgramDetails p) throws IOException{
		...
		buff.append("<style>.div{width:100%;height:100%;}.ui-widget-content{border:none}iframe { height: 100%;width: 100%;}img{width:100%;height:100%}video{width:100%;height:100%}.clock > img{width:auto; height:auto}");
		buff.append("</style></head><body>");
		//从中判断出图片地址，视频地址。
		String context=p.getContext();		
		buff.append(context.replaceAll("/upload_cloudfile/upload/image", "Images").replaceAll("/upload_cloudfile/upload/video", "Video").replaceAll("/CloudringAD/images/sys", "SceneImages").replaceAll("/upload_cloudfile/upload/doc","Doc").replaceAll(PropertiesUtils.getInstance().getValue("background-image")+"/upload_cloudfile/upload/bgpicture","Images"));	
		  if(!"".equals(program.getBagrmusic())&&program.getBagrmusic()!=null){
			  buff.append("<div id=\"lanren\"><div id=\"audio-btn\" class=\"on\" onclick=\"lanren.changeClass(this,'media')\">");
			  buff.append(" <audio loop=\"loop\" src=\""+program.getBagrmusic().replaceAll("/upload_cloudfile/upload/music", "Music")+"\" id=\"media\" preload=\"preload\"></audio></div></div>");
		...
		buff.append("<script type=\"text/javascript\">$(function(){$('a.media').media({width:800, height:600}); });</script>");
 ...
 public void createFile(Program program,String srcFile){
	...
	//判断背景图片是否有
		if(!"".equals(program.getBackground_picture())&&program.getBackground_picture()!=null){
			 String createSrc=srcFile+File.separator+"Images"+File.separator;
			 String dirFile=createSrc+program.getBackground_picture().replace("/upload_cloudfile/upload/bgpicture/", "");
