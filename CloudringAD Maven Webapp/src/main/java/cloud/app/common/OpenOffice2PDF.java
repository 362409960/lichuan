...
public class OpenOffice2PDF {
    ...
    /**
     * 根据操作系统的名称，获取OpenOffice.org 4的安装目录 如我的OpenOffice.org 4安装在：C:/Program
    ...
    public String getOfficeHome() {
        ...
        if (Pattern.matches("Linux.*", osName)) {
            return PropertiesUtils.getInstance().getValue("linux.openoffice");
        } else if (Pattern.matches("Windows.*", osName)) {
            return PropertiesUtils.getInstance().getValue("windows.openoffice");
        }
        return null;
    }
    /**
     ...
     * @return
     */
    public synchronized boolean openOffice2Pdf(String inputFilePath, String outputFilePath) {
    	 boolean flag = false;      
    	try {
    		  DefaultOfficeManagerConfiguration config = new DefaultOfficeManagerConfiguration();
    	        // 获取OpenOffice.org 4的安装目录
    	        ...
    	        config.setPortNumber(Integer.parseInt(PropertiesUtils.getInstance().getValue("portNumber")));
    	        // 启动OpenOffice的服务
    ...
    public static void main(String[] args) {
        ...
        office2pdf.openOffice2Pdf("E:\\doc\\魔镜2.0相册需求清单.xlsx","D:/1_" + new Date().getTime() + ".pdf" );
