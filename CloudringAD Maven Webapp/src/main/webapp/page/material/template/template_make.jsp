...
<input type="hidden" name="context" id="context"/><!--xiu  -->
<input type="hidden" name="context_video" id="context_video"/><!--yuan  -->
<input type="hidden" name="id" id="id" />
<input type="hidden" name="sonId" id="sonId" />
<input type="hidden" name="video_url" id="video_url" value="${template.video_url}"/>
<input type="hidden" name="imgae_url" id="imgae_url" value="${template.imgae_url}"/>
<input type="hidden" name="pdf_url" id="pdf_url" value="${template.pdf_url}"/>
...
<input type="hidden" name="video_stream" id="video_stream" /><!-- ip -->
<input type="hidden" name="videoPlace_url" id="videoPlace_url" value="${template.videoPlace_url}" />
<input type="hidden" id="background_picture" name="background_picture"/>


<div id="EditDiv">
  <div class="Toolbar" style="padding:2px 4px 0;">
    <div id="div3" class="css_normal left">
      <input id="backToMg" class="btnYellowPale" type="button"  value="返回" onclick="history.back(); return false;">
      ...
      <input id="program_name" type="text" value="最新节目-${now_time}" name="program_name"  maxlength="25">
      </span> <span>分辨率：</span> <span id="screenRate">
      <select id="sel_screenRate"  onChange="setScreenRate(this)" name="resolution">
         <option value="1024*768"  <c:if test="${template.resolution=='1024*768'}">selected</c:if>>1024*768</option>
        <option value="1280*720" <c:if test="${template.resolution=='1280*720'}">selected</c:if>>1280*720</option>
        <option value="1366*768" <c:if test="${template.resolution=='1366*768'}">selected</c:if>>1366*768</option>
        <option value="1920*1080" <c:if test="${template.resolution=='1920*1080'}">selected</c:if>>1920*1080</option>       
      </select>     
      </span> <span>节目类型：</span>
      <select id="program_type" name="program_type">
        <option value="普通节目" selected>普通节目</option>
        <option value="互动节目">互动节目</option>
      </select>
     <!--  <span id="interactTime"> <span>展示时长：</span>
      <input id="release_time" type="text" name="expiration">
      <span>秒</span> --> </span> <span>背景音乐：</span> <span id="bgMusic">
      <select id="bagrmusic" onChange="setbgMusic()" name="bagrmusic">
        <option value="" selected>请选择背景音乐</option>
          <c:forEach items="${mMusicList}" var="type">
...
<div id="div_editor" class="editor">
  ...
  <div id="panel" class="panel">
    ...
    <div class="panel-main" >
      <div class="wrapAll" id="wrapAll">
          <div id="main" class="main">${template.context_video}</div>
      </div>
    </div>
  </div>
  <div id="righttool" class="righttool">
    ...
    <div class="scene">
      ...
      <div> <span>当前: </span>
        <input id="scenes" class="inp-60" type="text" value="场景1" name="scenes">
        <br>
        <div style="height:3px"></div>
        <span>播放时间: </span>
        <input id="play_time" class="inp-25" type="text" value="15" name="play_time">
        秒 </div>
      <hr class="hr-w150">
      	...
      	<div style="height:180px;">
			<table class="tabs-table">
            	<tr>
                    ...
                    <td style="width:120px;">
                    	<input type="text" id="top"  value="" name="" class="inp-60">&nbsp;px
                    </td>
...
;(function(){
	var createScene=document.getElementById('input_createScene');
