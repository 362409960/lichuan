...
;(function(){
	...
	$widget.each(function(index, element) {
		...
		$(this).contextPopup({
			title:'功能编辑',
			items: [
			  {label:'删除',icon:'images/icon/delete.gif',enable:true,action:function(){oMain.removeChild($this);}, color:''},
			]
		});
    });	
	...
	EventUtil.addHandler(oPlace,'click',function(e){
		...
		oImg.src='images/video_control.png';
		oImg.className='img';
	...
	$(oMain).delegate('.ui-widget-content','dblclick',function(){
		...
		if(reg.test($id)){
			...
			var oInput=document.querySelectorAll('.layer-monitor input[type=text]');
