...
html5Clock = function(config){
	...
	clockDiv.each(function(index, element) {
		var $this=$(this);
       clockWidthHeight = config.clockWidthAndHeight;
		...
		$this.css({"height":clockWidthHeight + "px", "width":clockWidthHeight + "px", "position":"absolute"});
		...
		if(retina){
			...
			$this.append("<img class='bg' src=" + config.clockFaceHighResImg + " height="+clockWidthHeight+" width="+clockWidthHeight+" />");
			$this.append("<img class='hourHand' src=" + config.hourHandHighResImg + " />");
			$this.append("<img class='minuteHand' src=" + config.minuteHandHighResImg + " />");
			if(useSecondHand)$this.append("<img class='secondHand' src=" + config.secondHandHighResImg + " />");
			
		}else{
		
			$this.append("<img class='bg' src=" + config.clockFaceImg + " /><img class='hourHand' src=" + config.hourHandImg + " /><img class='minuteHand' src=" + config.minuteHandImg + " />");
			if(useSecondHand)$this.append("<img class='secondHand' src=" + config.secondHandImg + " />");
