...
;(function(){
	...
	$widget.each(function(index, element) {
		...
		if (program_name == null || trim(program_name) == '') {
			...
			var url = contextPath + '/template/save.do';
		...
		var index=layer.open({
			...
			btn1:function(){
				...
				$(oDiv).contextPopup({
					title:'功能编辑',
					items: [
					  {label:'删除',icon:contextPath+'/images/sys/icon/delete.gif',enable:true,action:function(){oMain.removeChild(oDiv);}, color:''},
					]
				});
	...
	EventUtil.addHandler(oDate,'click',function(e){
		...
		var prev=document.createElement('prev');
			...
			$(oDiv).contextPopup({
				title:'功能编辑',
				items: [
				  {label:'删除',icon:contextPath+'/images/icon/sys/delete.gif',enable:true,action:function(){oMain.removeChild(oDiv);}, color:''},
				]
			});
...
function preview(){
	...
	var url_pattern = contextPath + '/template/toPreview.do';
