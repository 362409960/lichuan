...
<div id="right_top">
	...
	<div id="right_font">
	  	...
	  	<span class="bfont">商品分类</span>
	</div>
	<div style="margin:20px 0;" align="center"></div>
	<form id="merchandiseCategoriesListForm" method="post">
		<table class="tableForm_L" style="margin-top:3px" width="99%" border="0" cellpadding="0" align="center" style="width: 80%; margin-bottom: 10px">
	       ...
	       <div id="tb" align="center" style="width:80%; margin-bottom:10px; margin-left:100px;">
	                                商品分类：<input class="easyui-text" name="name" style="width: 130px; margin-right: 30px;">
	...
	<script type="text/javascript">
		...
		$(function(){
			$dg = $('#dg').datagrid({
				title: "商品分类列表",
        		url: contextPath+"/admin/categories/listMerchandiseCategories.do",
				pagination : true,
				pageSize: 15,//每页显示的记录条数，默认为10
		        pageList: [10, 15, 20, 25, 30],//可以设置每页记录条数的列表
        		loadMsg:'加载中请稍后...',
        		queryParams: form2Json("merchandiseCategoriesListForm"), // 传查询条件
				columns : [[{
					field : 'id',
					checkbox : true
				},
        ...
        function editScene(){
        	var row = $('#dg').datagrid('getSelected');
           	...
           	if (rows.length == 1) {
           	 var id=row.id;         		
                url_pattern = "<%=request.getContextPath()%>/admin/categories/editMerchandiseCategories.do?id="+id;
        ...
        function newScene(){           
           url_pattern='<%=request.getContextPath()%>/admin/categories/addMerchandiseCategories.do';
      ...
      /*   function saveScene(){
        ...
        function destroyScene(){
            ...
            if (rows.length == 1) {
           		$.messager.confirm('删除','确定要删除？',function(r){  
                    if (r){  
                       $.post('<%=request.getContextPath()%>/admin/categories/deleteMerchandiseCategories.do',{id:row.id},function(result){
                            $('#dg').datagrid('reload');
                        },'json');
                   }
                   $('#dg').datagrid('reload');
		...
		function searchResult() {
	   		$('#dg').datagrid({ queryParams: form2Json("merchandiseCategoriesListForm") });
	   		//设置分页控件
		    var p = $('#dg').datagrid('getPager');
		    p.pagination({
		        pageSize: 15,//每页显示的记录条数，默认为10
