...
<body>
<div class="breadcrumb"><a href="<%=path%>/index.jsp">首页</a>&raquo;&nbsp;编辑商品属性</div>
<form id="inputForm"  method="post" action="<%=path%>/admin/attributes/edit.do">
<input type="hidden" name="id" id="id" value="${attr.id}"/>
	<table class="input">
    	<tr>
        	<th>绑定分类:</th>
            <td>
            	<select name="productCategoryId">
                	<c:forEach items="${category}" var="it" varStatus="st">
                           <c:choose>
                             <c:when test="${it.id==attr.id}"> <option value="${it.id}" selected="selected">${it.category_name}</option></c:when>
        ...
        <tr>
            ...
            <td><input type="text" name="attr_name" class="text" value="${attr.attr_name}" maxlength="200"></td>
        </tr>
        <tr>
        	<th>排序:</th>
            <td><input type="text" name="order_value" class="text" value="${attr.order_value}" maxlength="9"></td>
        </tr>
        <tr>
        	<th>&nbsp;</th>
        ...
        <tr>
            ...
            <td>
            	<table id="optionTable" class="item">
                    ...
                    <c:forEach items="${attr.options}" var="it" varStatus="st">
                    <tr>
                    	<td>
