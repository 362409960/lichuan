...
<body>
<div class="breadcrumb"><a href="<%=path%>/index.jsp">首页</a>&raquo;商品属性<span>(共${total}条记录)</span></div>
<form id="listForm" method="post" action="<%=path%>/admin/attributes/toSearch.do">
...
<input type="hidden" id="orderDirection" name="orderDirection" value="" />
	...
	<div class="bar">
    	<a class="iconButton" href="<%=path%>/admin/attributes/toAdd.do">
    ...
    <table id="listTable" class="list">
    	<tr>
          ...
          <c:forEach items="${attr.rows}" var="pIt" varStatus="st">
        <tr>
        	<td><input type="checkbox" name="ids" value="${pIt.id}"></td>
            <td>${pIt.attr_name}</td>
            <td>${pIt.productCategoryId}</td>
            <td>${pIt.att_options}</td>
            <td>${pIt.order_value}</td>
            <td><a href="<%=path%>/admin/attributes/toEdit.do?id=${pIt.id}">[编辑]</a>&nbsp; <a href="<%=path%>/admin/attributes/toCopy.do?id=${pIt.id}" >[复制]</a></td>
        </tr>
        </c:forEach>
    </table>
     <div class="pagination">
         <c:choose>
		     <c:when test="${attr.pageMax!=0}">
		     <c:choose>
				<c:when test="${attr.pageIndex+1==1}">
				   <c:choose>
				    <c:when test="${attr.pageIndex+1==attr.pageMax}">	
					 ...
					 <c:otherwise>
							...
							<a href="javascript: $.pageSkip(${attr.pageIndex+2});" class="nextPage">&nbsp;</a>
							<a href="javascript: $.pageSkip(${attr.pageMax});" class="lastPage">&nbsp;</a>
							<span class="pageSkip">
					        	共${attr.pageMax}页
					            <input id="pageNumber" name="pageNumber" value="${attr.pageIndex+1}" maxlength="9">
					           	<button type="submit">&nbsp;</button>
					        </span>
					 </c:otherwise>
					 </c:choose>
				...
				<c:when test="${attr.pageIndex+1==attr.pageMax}">
						...
						<a href="javascript: $.pageSkip(${attr.pageIndex});" class="previousPage">&nbsp;</a>
						<c:forEach var="i" begin="${p_begin}" end="${p_end}">
				            <c:choose>
				              <c:when test="${i==attr.pageIndex+1}"><span class="currentPage">${i}</span></c:when>
				              <c:otherwise><a href="javascript:$.pageSkip(${i});">${i}</a></c:otherwise>
...
$(function(){
	...
	var $checkedIds = $("#listTable input[name='ids']:enabled:checked");	
	  ...
	  $.ajax({
		url: contextPath+"/admin/attributes/deleteById.do",
		type: "POST",
