...
<body>
<div class="breadcrumb"><a href="<%=path%>/index.jsp">首页</a>&raquo;品牌列表<span>(共${total}条记录)</span></div>
<form id="listForm" method="post" action="<%=path%>/admin/band/toSearch.do">
	...
	<div class="bar">
    	<a class="iconButton" href="<%=path%>/admin/band/toAdd.do">
        ...
        <div class="buttonGroup">
            ...
            <div id="pageSizeMenu" class="dropdownMenu">
                ...
                <ul>
                	<c:choose>
                      <c:when test="${brand.pageSize=='10'}">
                         <li class="current" val="10">10</li>
	                    <li  val="20">20</li>
	                    <li val="50">50</li>  
                      </c:when>
                      <c:when test="${brand.pageSize=='20'}">
                         <li val="10">10</li>
    ...
    <table id="listTable" class="list">
    	<tr>
            ...
            <th><a href="javascript:;" class="sort" name="brand_name">名称</a></th>
            <th><a href="javascript:;" class="sort" name="logo">logo</a></th>
            <th><a href="javascript:;" class="sort" name="url">网址</a></th>
            <th><a href="javascript:;" class="sort" name="order_value">排序</a></th>
            <th><span>操作</span></th>
        </tr>
         <c:forEach items="${brand.rows}" var="pIt" varStatus="st">
        <tr>
        	<td><input type="checkbox" name="ids" value="${pIt.id}"></td>
            <td>${pIt.brand_name}</td>
            <td><a href="${pIt.logo}" target="_blank">查看</a></td>
            <td><a href="${pIt.url}" target="_blank">${pIt.url}</a></td>
            <td>${pIt.order_value}</td>
            <td><a href="<%=path%>/admin/band/toEdit.do?id=${pIt.id}">[编辑]</a>&nbsp;<!-- <a href="##" target="_blank">[查看]</a> --></td>
        </tr>
        </c:forEach>
    </table>
     <div class="pagination">
         <c:choose>
		     <c:when test="${brand.pageMax!=0}">
		     <c:choose>
				<c:when test="${brand.pageIndex+1==1}">
				   <c:choose>
				    <c:when test="${brand.pageIndex+1==brand.pageMax}">	
					 ...
					 <c:otherwise>
							...
							<c:forEach var="i" begin="${p_begin}" end="${p_end}">
					            <c:choose>
					              <c:when test="${i==brand.pageIndex+1}"><span class="currentPage">${i}</span></c:when>
					              <c:otherwise><a href="javascript:$.pageSkip(${i});">${i}</a></c:otherwise>
					            </c:choose>
					         </c:forEach>						
							<a href="javascript: $.pageSkip(${brand.pageIndex+2});" class="nextPage">&nbsp;</a>
							<a href="javascript: $.pageSkip(${brand.pageMax});" class="lastPage">&nbsp;</a>
							<span class="pageSkip">
					        	共${brand.pageMax}页
					            <input id="pageNumber" name="pageNumber" value="${brand.pageIndex+1}" maxlength="9">
					           	<button type="submit">&nbsp;</button>
					        </span>
					 </c:otherwise>
					 </c:choose>
				...
				<c:when test="${brand.pageIndex+1==brand.pageMax}">
						...
						<a href="javascript: $.pageSkip(${brand.pageIndex});" class="previousPage">&nbsp;</a>
						<c:forEach var="i" begin="${p_begin}" end="${p_end}">
				            <c:choose>
				              <c:when test="${i==brand.pageIndex+1}"><span class="currentPage">${i}</span></c:when>
				              <c:otherwise><a href="javascript:$.pageSkip(${i});">${i}</a></c:otherwise>
...
$(function(){
	...
	var $checkedIds = $("#listTable input[name='ids']:enabled:checked");	
	  ...
	  $.ajax({
		url: contextPath+"/admin/band/deleteBrandById.do",
		type: "POST",
