...
<body>
<div class="breadcrumb"><a href="<%=path%>/index.jsp">首页</a>&raquo;&nbsp;编辑商品参数</div>
<form id="inputForm"  method="post" action="<%=path%>/admin/parameters/edit.do">
<input type="hidden" name="id" id="id" value="${paramerter.id}"/>
	<table class="input">
    	<tr>
        	<th>绑定分类:</th>
            <td>
            	<select name="productCategoryId">
                	<c:forEach items="${category}" var="it" varStatus="st">
                           <c:choose>
                             <c:when test="${it.id==paramerter.id}"> <option value="${it.id}" selected="selected">${it.category_name}</option></c:when>
        ...
        <tr>
            ...
            <td><input type="text" name="para_name" class="text" value="${paramerter.para_name}" maxlength="200"></td>
        </tr>
        <tr>
        	<th>排序:</th>
            <td><input type="text" name="order_value" class="text" value="${paramerter.order_value}" maxlength="9"></td>
        </tr>
        <tr>
        	<th>&nbsp;</th>
            <td><a id="addOptionButton" class="button" href="javascript:;">增加商品参数</a></td>
        </tr>
        <tr>
        	<th>&nbsp;</th>
            <td>
            	<table id="optionTable" class="item">
                    ...
                    <c:forEach items="${paramerter.options}" var="it" varStatus="st">
                    <tr>
                    	<td>
