...
<body>
<div class="breadcrumb"><a href="<%=path%>/index.jsp">首页</a>&raquo;商品参数<span>(共${total}条记录)</span></div>
<form id="listForm" method="post" action="<%=path%>/admin/parameters/toSearch.do">
...
<input type="hidden" id="orderDirection" name="orderDirection" value="" />
	...
	<div class="bar">
    	<a class="iconButton" href="<%=path%>/admin/parameters/toAdd.do">
        ...
        <div class="buttonGroup">
            ...
            <div id="pageSizeMenu" class="dropdownMenu">
                ...
                <ul>
                	<c:choose>
                      <c:when test="${attr.pageSize=='10'}">
                         <li class="current" val="10">10</li>
	                    <li  val="20">20</li>
	                    <li val="50">50</li>  
                      </c:when>
                      <c:when test="${attr.pageSize=='20'}">
                         <li val="10">10</li>
    ...
    <table id="listTable" class="list">
    	<tr>
            ...
            <th><a href="javascript:;" class="sort" name="attr_name">名称</a></th>
          ...
          <c:forEach items="${paramerter.rows}" var="pIt" varStatus="st">
        <tr>
        	<td><input type="checkbox" name="ids" value="${pIt.id}"></td>
            <td>${pIt.para_name}</td>
            <td>${pIt.productCategoryId}</td>
            <td>${pIt.par_options}</td>
            <td>${pIt.order_value}</td>
            <td><a href="<%=path%>/admin/parameters/toEdit.do?id=${pIt.id}">[编辑]</a>&nbsp; <a href="<%=path%>/admin/parameters/toCopy.do?id=${pIt.id}" >[复制]</a></td>
        </tr>
        </c:forEach>
    </table>
     <div class="pagination">
         <c:choose>
		     <c:when test="${paramerter.pageMax!=0}">
		     <c:choose>
				<c:when test="${paramerter.pageIndex+1==1}">
				   <c:choose>
				    <c:when test="${paramerter.pageIndex+1==paramerter.pageMax}">	
					 ...
					 <c:otherwise>
							...
							<c:forEach var="i" begin="${p_begin}" end="${p_end}">
					            <c:choose>
					              <c:when test="${i==paramerter.pageIndex+1}"><span class="currentPage">${i}</span></c:when>
					              <c:otherwise><a href="javascript:$.pageSkip(${i});">${i}</a></c:otherwise>
					            </c:choose>
					         </c:forEach>						
							<a href="javascript: $.pageSkip(${paramerter.pageIndex+2});" class="nextPage">&nbsp;</a>
							<a href="javascript: $.pageSkip(${paramerter.pageMax});" class="lastPage">&nbsp;</a>
							<span class="pageSkip">
					        	共${paramerter.pageMax}页
					            <input id="pageNumber" name="pageNumber" value="${paramerter.pageIndex+1}" maxlength="9">
					           	<button type="submit">&nbsp;</button>
					        </span>
					 </c:otherwise>
					 </c:choose>
				...
				<c:when test="${paramerter.pageIndex+1==paramerter.pageMax}">
						...
						<a href="javascript: $.pageSkip(${paramerter.pageIndex});" class="previousPage">&nbsp;</a>
						<c:forEach var="i" begin="${p_begin}" end="${p_end}">
				            <c:choose>
				              <c:when test="${i==attr.pageIndex+1}"><span class="currentPage">${i}</span></c:when>
				              <c:otherwise><a href="javascript:$.pageSkip(${i});">${i}</a></c:otherwise>
				            </c:choose>
				         </c:forEach>
...
$(function(){
	...
	var $checkedIds = $("#listTable input[name='ids']:enabled:checked");	
	  ...
	  $.ajax({
		url: contextPath+"/admin/parameters/deleteById.do",
		type: "POST",
